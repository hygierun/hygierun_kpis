            ...
            with st.spinner("⏳ Traitement en cours..."):
                ...
                with tempfile.TemporaryDirectory() as tmpdir:
                    ...
                    if not df_delais.empty:
                        # [conversions et filtres comme avant...]
                        ...
                        for col_idx, header in enumerate(headers, 1):
                            cell = ws_delais.cell(row=1, column=col_idx)
                            cell.value = header
                            cell.font = Font(bold=True, color="FFFFFF")
                            cell.fill = PatternFill(start_color="366092", end_color="366092", fill_type="solid")
                        ...
                        for row_idx, (_, row) in enumerate(df_filtered.iterrows(), 2):
                            for col_idx, (col_name, value) in enumerate(row.items(), 1):
                                cell = ws_delais.cell(row=row_idx, column=col_idx)
                                cell.value = value
                                # Format Tournée en nombre (pas datetime)
                                if col_name == 'Tournée' and isinstance(value, (int, float)):
                                    cell.number_format = '0'
                        
                        st.info(f"✅ Feuille 'Delais': {len(df_filtered)} lignes (filtrées)")
                    
                    # Créer feuille "EnAttente" - SEULEMENT les lignes du calcul
                    ...
                    if not df_en_attente.empty:
                        ...
                        headers = list(df_filtered.columns)
                        for col_idx, header in enumerate(headers, 1):
                            cell = ws_en_attente.cell(row=1, column=col_idx)
                            cell.value = header
                            cell.font = Font(bold=True, color="FFFFFF")
                            cell.fill = PatternFill(start_color="366092", end_color="366092", fill_type="solid")
                        ...
                        for row_idx, (_, row) in enumerate(df_filtered.iterrows(), 2):
                            for col_idx, (col_name, value) in enumerate(row.items(), 1):
                                ws_en_attente.cell(row=row_idx, column=col_idx).value = value
                        
                        st.info(f"✅ Feuille 'EnAttente': {len(df_filtered)} lignes (filtrées)")
